...
class Lexer {
  ...
  lex() {
    ...
    while (!this.eof()) {
      ...
      // Compound assignment: +=  -=  *=  /=  (must come before ++ and single-char)
      if ((c === "+" || c === "-" || c === "*" || c === "/") && this.src[this.i + 1] === "=") {
        const v = this.next() + this.next();
        out.push(this.makeToken("symbol", v, startLine, startCol, startIdx));
        continue;
...
class Parser {
  ...
  parseStmt() {
    ...
    if (this.peek().t === "symbol" && (this.peek().v === "++" || this.peek().v === "--")) {
      ...
      if (this.matchSymbol(";")) {}
      return { kind: "assign", target: e, expr: { kind: "bin", op: op[0], left: e, right: { kind: "num", value: 1 } } };
    }

    // Compound assignment: +=  -=  *=  /=
    const compoundOps = ["+=", "-=", "*=", "/="];
    const tok = this.peek();
    if (tok.t === "symbol" && compoundOps.includes(tok.v)) {
      const op = this.next().v;          // consume "+=" etc.
      const rhs = this.parseExpr();
      if (this.matchSymbol(";")) {}
      // Desugar: x += y  →  assign(x, x + y)
      return { kind: "assign", target: e, expr: { kind: "bin", op: op[0], left: e, right: rhs } };
    }
...
const Op = {
  CONST: "CONST", LOAD: "LOAD", STORE: "STORE", POP: "POP", DUP: "DUP",
...
class Compiler {
  constructor() { 
    ...
    this.loopStack = []; 
    this.omitHalt = false;
    this.tempVarCounter = 0; // deterministic temp var names, no collisions
  }
  ...
  emitStmt(s) {
    switch (s.kind) {
      ...
      case "for": {
        const iterName = `__iter_${this.tempVarCounter++}`;
        const idxName  = `__i_${this.tempVarCounter++}`;   // deterministic, no collision risk
  ...
  emitExpr(e) {
    switch (e.kind) {
      ...
      case "bin": {
        if (e.op === "&&") {
          // Truthy left: DUP[L,L], JMPF pops (doesn't jump)->[L], POP->[],  emit right->[R], end->[R]
          // Falsy  left: DUP[L,L], JMPF pops (jumps to end) ->[L], end->[L]
        ...
        if (e.op === "||") {
          // DUP left so JMPF can consume one copy while the other stays on stack.
          // Truthy: DUP gives [L,L], JMPF does NOT jump, pops top -> [L], JMP to end -> result is L.
          // Falsy:  DUP gives [L,L], JMPF DOES jump, pops top -> [L], at B: POP -> [], push right -> [R].
          this.emitExpr(e.left);
          this.emit({ op: Op.DUP });
          const jmpfPos = this.code.length; this.emit({ op: Op.JMPF, arg: null });
