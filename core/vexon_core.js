...
class Lexer {
  ...
  lex() {
    ...
    while (!this.eof()) {
      ...
      // Increment / decrement: ++ --
      if ((c === "+" && this.src[this.i + 1] === "+") || (c === "-" && this.src[this.i + 1] === "-")) {
        const v = this.next() + this.next();
        out.push(this.makeToken("symbol", v, startLine, startCol, startIdx));
        continue;
      }
      ...
      if (isAlpha(c)) {
        ...
        // f-string: f"..." or f'...'
        if (id === "f" && !this.eof() && (this.peek() === '"' || this.peek() === "'")) {
          const q = this.next(); // consume opening quote
          let raw = "";
          let depth = 0; // track nested braces inside expressions
          while (!this.eof()) {
            const ch = this.next();
            if (ch === "\\" && !this.eof()) {
              const esc = this.next();
            ...
            if (ch === "}" ) { depth--; raw += ch; continue; }
            if (ch === q && depth === 0) break; // end of fstring
            raw += ch;
          }
          out.push(this.makeToken("fstring", raw, startLine, startCol, startIdx));
          continue;
        }
...
class Parser {
  ...
  parseStmt() {
    ...
    // Postfix ++ and -- (desugar: x++ => x = x + 1)
    if (this.peek().t === "symbol" && (this.peek().v === "++" || this.peek().v === "--")) {
      const op = this.next().v[0]; // "+" or "-"
      if (this.matchSymbol(";")) {}
      return { kind: "assign", target: e, expr: { kind: "bin", op, left: e, right: { kind: "num", value: 1 } } };
    }
    // Compound assignment: += -= *= /=
    const compoundOps = ["+=", "-=", "*=", "/="];
    const ctok = this.peek();
    if (ctok.t === "symbol" && compoundOps.includes(ctok.v)) {
      const op = this.next().v[0]; // "+", "-", "*", "/"
      const rhs = this.parseExpr();
      if (this.matchSymbol(";")) {}
      return { kind: "assign", target: e, expr: { kind: "bin", op, left: e, right: rhs } };
    }
 ...
 parseFn() {
  ...
  // Handles chained .prop, [idx], and () calls on any primary expression
  _parsePostfix(node) {
    while (true) {
      if (this.peek().t === "symbol" && this.peek().v === "(") {
        this.next();
  ...
  parsePrimary() {
      ...
      if (tk.t === "number") { this.next(); return this._parsePostfix({ kind: "num", value: Number(tk.v) }); }
      if (tk.t === "string") { this.next(); return this._parsePostfix({ kind: "str", value: tk.v }); }

      if (tk.t === "fstring") {
        this.next();
        // Split raw fstring into literal and expression parts
        const raw = tk.v;
        const parts = [];
        let i = 0;
        let lit = "";
        while (i < raw.length) {
          if (raw[i] === "{") {
            if (lit.length > 0) { parts.push({ type: "str", value: lit }); lit = ""; }
            let depth = 1;
            let exprSrc = "";
            i++;
            ...
            // Re-lex and re-parse the expression inside {}
            const exLexer = new Lexer(exprSrc);
            const exTokens = exLexer.lex();
            const exParser = new Parser(exTokens, exprSrc);
            const exprAst = exParser.parseExpr();
            parts.push({ type: "expr", expr: exprAst });
          } else {
            lit += raw[i++];
          }
        }
        if (lit.length > 0) parts.push({ type: "str", value: lit });
        return this._parsePostfix({ kind: "interp", parts });
      }
      ...
      if (tk.t === "keyword" && tk.v === "this") {
        this.next();
        return this._parsePostfix({ kind: "var", name: "this" });
      }
    ...
    if (tk.t === "symbol" && tk.v === "[") {
      ...
      return this._parsePostfix({ kind: "array", elements: elems });
    }
    ...
    if (tk.t === "symbol" && tk.v === "{") {
      ...
      return this._parsePostfix({ kind: "obj", entries });
    }
...
const Op = {
  CONST: "CONST", LOAD: "LOAD", STORE: "STORE", POP: "POP", TOSTR: "TOSTR",
...
class Compiler {
  ...
  collectFreeVars(params, bodyStmts) {
    const locallyDefined = new Set(
      params.map(p => (typeof p === 'object' ? p.name : p))
    );
    const free = new Set();
    ...
    const scanExpr = (e) => {
      ...
      switch (e.kind) {
        case 'var':
          if (!locallyDefined.has(e.name)) free.add(e.name);
          break;
        case 'fn': {
          // Anonymous fn expression — collect its free vars and bubble up
          // those not defined in the current scope
          if (e.name) locallyDefined.add(e.name);
          const nested = this.collectFreeVars(e.params || [], e.body || []);
          for (const n of nested) { if (!locallyDefined.has(n)) free.add(n); }
          break;
        }
        case 'call':
          scanExpr(e.callee); (e.args || []).forEach(scanExpr);
          break;
        case 'bin':
        ...
        case 'unary':
          ...
          break;
        case 'index':
          scanExpr(e.target); scanExpr(e.index);
          break;
        case 'prop':
          scanExpr(e.target);
          break;
        case 'array':
          (e.elements || []).forEach(scanExpr);
          break;
        case 'obj':
          (e.entries || []).forEach(en => scanExpr(en.value));
          break;
        default: break;
      }
    };
    ...
    const scanStmt = (s) => {
      ...
      switch (s.kind) {
        case 'let':
          scanExpr(s.expr);
          locallyDefined.add(s.name);
          break;
        case 'assign':
          scanExpr(s.target); scanExpr(s.expr);
          break;
        case 'expr':
          scanExpr(s.expr);
          break;
        case 'return':
          if (s.expr) scanExpr(s.expr);
          break;
        case 'if':
          scanExpr(s.cond);
          (s.then||[]).forEach(scanStmt);
          (s.otherwise||[]).forEach(scanStmt);
          break;
        case 'while':
          scanExpr(s.cond);
        ...
        case 'for':
          scanExpr(s.iterable);
          locallyDefined.add(s.iterator);
          (s.body||[]).forEach(scanStmt);
          break;
        case 'fn': {
          // Named fn statement — collect its free vars, bubble up, then
          // add its name to locally defined
          const nested = this.collectFreeVars(s.params || [], s.body || []);
          for (const n of nested) { if (!locallyDefined.has(n)) free.add(n); }
          if (s.name) locallyDefined.add(s.name);
          break;
        }
        case 'try':
          (s.tryBody||[]).forEach(scanStmt);
          (s.catchBody||[]).forEach(scanStmt);
          break;
        case 'throw':
          scanExpr(s.expr);
          break;
    ...
    bodyStmts.forEach(scanStmt);
    return [...free];
  }



  collectDeclaredLocals(bodyStmts) {
    const locals = new Set();
    const scan = (s) => {
      if (!s) return;
      if (s.kind === 'let')   { locals.add(s.name); }
      if (s.kind === 'for')   { locals.add(s.iterator); s.body.forEach(scan); }
      if (s.kind === 'if')    { (s.then||[]).forEach(scan); (s.otherwise||[]).forEach(scan); }
      if (s.kind === 'while') { (s.body||[]).forEach(scan); }
      if (s.kind === 'try')   { (s.tryBody||[]).forEach(scan); (s.catchBody||[]).forEach(scan); if (s.errVar) locals.add(s.errVar); }
      // don't recurse into nested fn bodies — they have their own scope
    };
    bodyStmts.forEach(scan);
    return [...locals];
  }
    ...
    switch (s.kind) {
      ...
      case "class": {
        ...
        for (const m of s.methods) {
          ...
          const declaredLocals = this.collectDeclaredLocals(m.body);
          const fnObj = { params: ["this", ...m.params], code: bytecode.code, consts: bytecode.consts, declaredLocals };
          this.emit({ op: Op.DUP });                                  // keep class obj on stack
          this.emit({ op: Op.CONST, arg: this.addConst(m.name) });
          this.emit({ op: Op.CONST, arg: this.addConst(fnObj) });
          this.emit({ op: Op.SETPROP });                              // pops 3, pushes val
          this.emit({ op: Op.POP });                                  // discard pushed val, classObj stays
        }
        this.emit({ op: Op.STORE, arg: this.addConst(s.name) });
        break;
      }
      case "fn": {
        ...
        const freeVars = this.collectFreeVars(s.params, s.body);
        const declaredLocals = this.collectDeclaredLocals(s.body);
        const funcObj = { params: s.params, code: bytecode.code, consts: bytecode.consts, freeVars, declaredLocals };
        if (freeVars.length > 0) {
          this.emit({ op: Op.CLOSURE, arg: this.addConst(funcObj) });
        } else {
  ...
  emitExpr(e) {
    switch (e.kind) {
      ...
      case "interp": {
        // Evaluate each part and concatenate into a single string
        const toStr = (v) => v === null ? "null" : String(v);
        if (e.parts.length === 0) {
          this.emit({ op: Op.CONST, arg: this.addConst("") });
          break;
        }
        // Emit first part
        const first = e.parts[0];
        if (first.type === "str") {
          this.emit({ op: Op.CONST, arg: this.addConst(first.value) });
        } else {
          this.emitExpr(first.expr);
          this.emit({ op: Op.TOSTR });
        }
        // Emit remaining parts and ADD each to the accumulator
        for (let pi = 1; pi < e.parts.length; pi++) {
          const part = e.parts[pi];
          if (part.type === "str") {
            this.emit({ op: Op.CONST, arg: this.addConst(part.value) });
          } else {
            this.emitExpr(part.expr);
            this.emit({ op: Op.TOSTR });
          }
          this.emit({ op: Op.ADD });
        }
      ...
      case "bin": {
        if (e.op === "&&") {
          // DUP so JMPF doesn't consume the falsy value we need as the result
          // emit left → [left]
          this.emitExpr(e.left);
          // DUP → [left, left]
          this.emit({ op: Op.DUP });
          // JMPF false_path: pops dup; truthy: stack=[left], continue; falsy: stack=[left], jump
          const jmpfPos = this.code.length; this.emit({ op: Op.JMPF, arg: null });
          // Truthy path: pop left, evaluate right
          this.emit({ op: Op.POP });
          this.emitExpr(e.right);
          const jmpEnd = this.code.length; this.emit({ op: Op.JMP, arg: null });
          // Falsy path: left(falsy) is already on stack
          this.code[jmpfPos].arg = this.code.length;
          this.code[jmpEnd].arg = this.code.length;
          break;
        }
        if (e.op === "||") {
          // Correct short-circuit: DUP so JMPF doesn't consume the truthy value
          // emit a → [a]
          this.emitExpr(e.left);
          // DUP → [a, a]
          this.emit({ op: Op.DUP });
          // JMPF else → pops dup; truthy: no jump, stack=[a]; falsy: jump to else, stack=[a]
          const jmpfPos = this.code.length; this.emit({ op: Op.JMPF, arg: null });
          // Truthy path: [a] on stack — jump to end
          const jmpToEnd = this.code.length; this.emit({ op: Op.JMP, arg: null });
          // Falsy path (else): pop the falsy a, evaluate b
          this.code[jmpfPos].arg = this.code.length;
          this.emit({ op: Op.POP });
          this.emitExpr(e.right);
      ...
      case "fn": {
        ...
        const freeVars = this.collectFreeVars(e.params, e.body);
        const declaredLocals = this.collectDeclaredLocals(e.body);
        const funcObj = { params: e.params, code: bytecode.code, consts: bytecode.consts, freeVars, declaredLocals };
        if (freeVars.length > 0) {
          this.emit({ op: Op.CLOSURE, arg: this.addConst(funcObj) });
        } else {
          this.emit({ op: Op.CONST, arg: this.addConst(funcObj) });
...
// All closures created in the same frame share the same UpvalCell for
// each captured variable, so mutations in one closure are visible in all.
class UpvalCell { constructor(v) { this.v = v; } }
  ...
  async run() {
    ...
    while (this.frames.length > 0) {
      ...
      if (frame.ip >= code.length) {
        if (frame.isConstructor && this.frames.length > 0) {
          this.push(frame.locals.get("this"));
        }
        this.frames.pop();
        continue;
      }
      ...
      try {
        ...
        switch (inst.op) {
          ...
          case Op.LOAD: {
            ...
            // Check closure captures first, then normal resolution
            if (frame.captures && frame.captures.has(name)) {
              const _cv = frame.captures.get(name); this.push(_cv instanceof UpvalCell ? _cv.v : _cv);
            } else {
          ...
          case Op.STORE: {
            ...
            const val = this.pop();
            if (frame.captures && frame.captures.has(name)) {
              const _sc = frame.captures.get(name); if (_sc instanceof UpvalCell) { _sc.v = val; } else { frame.captures.set(name, val); }
            } else if (frame.locals && frame.locals.has(name)) {
              frame.locals.set(name, val); if (frame.closureScope && frame.closureScope.has(name)) frame.closureScope.get(name).v = val;
            } else if (!frame.isGlobal && frame.locals && frame.locals.has(name)) {
              // Name was pre-seeded as a declared local — write here
          ...
          case Op.TOSTR: {
            const v = this.pop();
            this.push(v === null ? "null" : String(v));
            break;
          }
          ...
          case Op.NEWOBJ: {
            ...
            if (typeof target === 'string' && typeof idx === 'string') {
              const strMethods = {
                split:       { builtin: true, call: (args) => target.split(args[0] !== undefined ? args[0] : '') },
                replace:     { builtin: true, call: (args) => target.replace(args[0], args[1]) },
                replaceAll:  { builtin: true, call: (args) => target.replaceAll(String(args[0]), String(args[1])) },
                toUpperCase: { builtin: true, call: ()     => target.toUpperCase() },
                toLowerCase: { builtin: true, call: ()     => target.toLowerCase() },
                trim:        { builtin: true, call: ()     => target.trim() },
                trimStart:   { builtin: true, call: ()     => target.trimStart() },
                trimEnd:     { builtin: true, call: ()     => target.trimEnd() },
                startsWith:  { builtin: true, call: (args) => target.startsWith(String(args[0])) },
                endsWith:    { builtin: true, call: (args) => target.endsWith(String(args[0])) },
                includes:    { builtin: true, call: (args) => target.includes(String(args[0])) },
                indexOf:     { builtin: true, call: (args) => target.indexOf(String(args[0])) },
                slice:       { builtin: true, call: (args) => target.slice(args[0], args[1]) },
                substring:   { builtin: true, call: (args) => target.substring(args[0], args[1]) },
                repeat:      { builtin: true, call: (args) => target.repeat(Number(args[0])) },
                padStart:    { builtin: true, call: (args) => target.padStart(Number(args[0]), args[1] !== undefined ? String(args[1]) : ' ') },
                padEnd:      { builtin: true, call: (args) => target.padEnd(Number(args[0]),   args[1] !== undefined ? String(args[1]) : ' ') },
                charAt:      { builtin: true, call: (args) => target.charAt(Number(args[0])) },
                charCodeAt:  { builtin: true, call: (args) => target.charCodeAt(Number(args[0])) },
                toNumber:    { builtin: true, call: ()     => Number(target) },
                length:      target.length,  // also expose .length as a plain value
              };
              if (Object.prototype.hasOwnProperty.call(strMethods, idx)) {
                this.push(strMethods[idx]);
                break;
              }
            }

            // Array method dispatch
            if (Array.isArray(target) && typeof idx === 'string') {
              const arrMethods = {
                push:    { builtin: true, call: (args) => { target.push(...args); return target.length; } },
                pop:     { builtin: true, call: ()     => target.pop() ?? null },
                shift:   { builtin: true, call: ()     => target.shift() ?? null },
                unshift: { builtin: true, call: (args) => target.unshift(...args) },
                reverse: { builtin: true, call: ()     => { target.reverse(); return target; } },
                join:    { builtin: true, call: (args) => target.join(args[0] !== undefined ? String(args[0]) : ',') },
                slice:   { builtin: true, call: (args) => target.slice(args[0], args[1]) },
                indexOf: { builtin: true, call: (args) => target.indexOf(args[0]) },
                includes:{ builtin: true, call: (args) => target.includes(args[0]) },
                map:     { builtin: true, call: (args) => {
                  const fn = args[0]; const out = [];
                  for (const item of target) {
                    const frame = { code: fn.code, consts: fn.consts, ip: 0, locals: new Map(), isGlobal: false, tryStack: [] };
                    frame.locals.set(fn.params[0], item);
                    this.frames.push(frame);
                    // run synchronously by draining the frame
                    while (this.frames[this.frames.length-1] === frame) {
                      const inst2 = frame.code[frame.ip++];
                      if (!inst2 || inst2.op === 'HALT' || inst2.op === 'RET') {
                        const v = inst2 && inst2.op === 'RET' && this.stack.length ? this.pop() : null;
                        this.frames.pop(); out.push(v); break;
                      }
                      // fallback: just run through it
                      if (inst2.op === 'RET') { const v = this.stack.length ? this.pop() : null; this.frames.pop(); out.push(v); break; }
                    }
                  }
                  return out;
                }},
                filter:  { builtin: true, call: (args) => {
                  const fn = args[0]; const out = [];
                  for (const item of target) {
                    const frame = { code: fn.code, consts: fn.consts, ip: 0, locals: new Map(), isGlobal: false, tryStack: [] };
                    frame.locals.set(fn.params[0], item);
                    ...
                    while (this.frames[this.frames.length-1] === frame) {
                      const inst2 = frame.code[frame.ip++];
                      if (!inst2 || inst2.op === 'RET') { const v = this.stack.length ? this.pop() : null; this.frames.pop(); if (v) out.push(item); break; }
                      if (inst2.op === 'HALT') { this.frames.pop(); break; }
                    }
                  }
                  return out;
                }},
                find:    { builtin: true, call: (args) => {
                  const fn = args[0];
                  for (const item of target) {
                    const frame = { code: fn.code, consts: fn.consts, ip: 0, locals: new Map(), isGlobal: false, tryStack: [] };
                    frame.locals.set(fn.params[0], item);
                    ...
                    while (this.frames[this.frames.length-1] === frame) {
                      const inst2 = frame.code[frame.ip++];
                      if (!inst2 || inst2.op === 'RET') { const v = this.stack.length ? this.pop() : null; this.frames.pop(); if (v) return item; break; }
                      if (inst2.op === 'HALT') { this.frames.pop(); break; }
                    }
                  }
                  return null;
                }},
                some:    { builtin: true, call: (args) => {
                  const fn = args[0];
                  for (const item of target) {
                    const frame = { code: fn.code, consts: fn.consts, ip: 0, locals: new Map(), isGlobal: false, tryStack: [] };
                    frame.locals.set(fn.params[0], item);
                    ...
                    while (this.frames[this.frames.length-1] === frame) {
                      const inst2 = frame.code[frame.ip++];
                      if (!inst2 || inst2.op === 'RET') { const v = this.stack.length ? this.pop() : null; this.frames.pop(); if (v) return true; break; }
                      if (inst2.op === 'HALT') { this.frames.pop(); break; }
                    }
                  }
                  return false;
                }},
                every:   { builtin: true, call: (args) => {
                  const fn = args[0];
                  for (const item of target) {
                    const frame = { code: fn.code, consts: fn.consts, ip: 0, locals: new Map(), isGlobal: false, tryStack: [] };
                    frame.locals.set(fn.params[0], item);
                    ...
                    while (this.frames[this.frames.length-1] === frame) {
                      const inst2 = frame.code[frame.ip++];
                      if (!inst2 || inst2.op === 'RET') { const v = this.stack.length ? this.pop() : null; this.frames.pop(); if (!v) return false; break; }
                      if (inst2.op === 'HALT') { this.frames.pop(); break; }
                    }
                  }
                  return true;
                }},
                sort:    { builtin: true, call: (args) => {
                  if (!args[0]) { target.sort(); return target; }
                  // comparator fn support can be added later
                  target.sort(); return target;
                }},
                length:  target.length,
              };
              if (Object.prototype.hasOwnProperty.call(arrMethods, idx)) {
                this.push(arrMethods[idx]);
                break;
              }
            }

            // Auto-bind this when accessing a Vexon method on a plain object
            if (target && typeof target === "object" && !Array.isArray(target)) {
              const _meth = target[idx];
              if (_meth && _meth.code && Array.isArray(_meth.code) &&
                  _meth.params && _meth.params[0] === "this") {
                this.push(Object.assign({}, _meth, { _boundThis: target }));
                break;
              }
            }
            // Auto-bind native JS methods so server.get(), server.post() etc. work
            try {
              const _nval = target[idx];
              if (typeof _nval === 'function') {
                this.push({ builtin: true, call: (args) => _nval.apply(target, args) });
              } else {
                this.push(_nval);
              }
            } catch (e) {
          ...
          case Op.CALL: {
            ...
            if (callee && typeof callee === "object" && callee.init && callee.init.code) {
              ...
              funcFrame.isConstructor = true;
              this.frames.push(funcFrame);
              // Copy all non-init methods from the class to the instance
              for (const [key, val] of Object.entries(callee)) {
                if (key !== "init" && val && val.code) {
                  obj[key] = val;
                }
              }
              break;
            ...
            if (callee && callee.code && Array.isArray(callee.code)) {
              const funcFrame = {
                ...
                ip: 0,
                locals: new Map(),
                captures: callee.captures || null,
                baseDir: frame.baseDir,
                isGlobal: false,
                tryStack: [],
                module: callee.__module || null
              };
              // Pre-seed locals with declared variable names (undefined) so
              // STORE knows which names belong here vs to the outer/global scope.
              for (const localName of (callee.declaredLocals || [])) {
                funcFrame.locals.set(localName, undefined);
              }
              if (callee._boundThis !== undefined) {
                funcFrame.locals.set(callee.params[0], callee._boundThis);
                for (let i = 1; i < (callee.params ? callee.params.length : 0); i++) {
                  funcFrame.locals.set(callee.params[i], args[i-1]);
                }
              } else {
          ...
          case Op.HALT: {
            if (frame.isGlobal) {
              ...
              this.frames.pop();
              if (this.frames.length > 0) {
                this.push(frame.isConstructor ? frame.locals.get("this") : undefined);
                break;
              } else {
                return null;
              }
          ...
          case Op.CLOSURE: {
            // Build a closure with shared mutable UpvalCells.
            // All closures created in the same frame share the same cell
            // for each captured variable — mutations in one are visible in all.
            const proto = frame.consts[inst.arg];
            const closure = Object.assign({}, proto);
            closure.captures = new Map();
            if (!frame.closureScope) frame.closureScope = new Map();
            for (const name of (proto.freeVars || [])) {
              let cell = null;
              if (frame.closureScope.has(name)) {
                // Reuse existing cell — shared with sibling closures from this frame
                cell = frame.closureScope.get(name);
              } else if (frame.captures && frame.captures.has(name)) {
                // Propagate a cell from an outer closure's captures
                const _outer = frame.captures.get(name);
                cell = (_outer instanceof UpvalCell) ? _outer : new UpvalCell(_outer);
                frame.closureScope.set(name, cell);
              } else if (frame.locals && frame.locals.has(name)) {
                // Create a new cell seeded with the current local value
                cell = new UpvalCell(frame.locals.get(name));
                frame.closureScope.set(name, cell);
              }
              // Variables not found locally are resolved live via globals
              if (cell !== null) closure.captures.set(name, cell);
            }
            this.push(closure);
            break;
          }
      ...
      try {
        ...
        if (this.routes.has(routeKey)) {
          ...
          for (const [key, h] of this.routes.entries()) {
            ...
            const method = key.slice(0, _ci);
            const pattern = key.slice(_ci + 1);
