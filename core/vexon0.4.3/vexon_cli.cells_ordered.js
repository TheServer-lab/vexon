...
async function runFile(filePath, options = {}) {
  ...
  try {
    ...
    let cellFiles = [];
    try {
      cellFiles = fs.readdirSync(programDir)
        .filter(f => /^vexon_cell.*\.js$/.test(f))
        .map(f => path.join(programDir, f));
    } catch (e) {}
    return new VM(consts, code, {
      baseDir: programDir,
      debug: !!options.debug
    });
  })();
...
function createWindow() {
  ...
  return new VM(bytecode.consts, bytecode.code, { baseDir: programDir });
})();
...
async function runElectronRuntime(filePath) {
  ...
  app.whenReady().then(() => {
    ...
    try {
      ...
      let cellFiles = [];
      try { cellFiles = fs.readdirSync(programDir).filter(f => /^vexon_cell.*\.js$/.test(f)).map(f => path.join(programDir, f)); } catch(e) {}
      return new VM(compiled.consts, compiled.code, { baseDir: programDir });
    })();
