...
class VM {
  constructor(consts = [], code = [], options = {}) {
    ...
    // --- cell loader: load cells bundled with core (next to this file) ---
    // discover any numbered cell files next to vexon_core.js (vexon_cell1.js, vexon_cell2.js ...)
    this._cellPaths = [];
    try {
      const fsLocal = require("fs");
      const pathLocal = require("path");
      const coreDir = __dirname;
      const files = fsLocal.readdirSync(coreDir)
        .filter(f => {
          // match vexon_cellN.js where N is one or more digits
          return /^vexon_cell\d+\.js$/i.test(f);
        })
        .map(f => {
          const m = f.match(/^vexon_cell(\d+)\.js$/i);
          return { file: f, n: m ? parseInt(m[1], 10) : 0 };
        })
        .sort((a, b) => a.n - b.n)
        .map(o => pathLocal.join(coreDir, o.file));
      for (const f of files) this._cellPaths.push(f);
    } catch (e) { if (this.debug) console.warn("vexon: core cell discovery failed", e); }

    this.setupBuiltins();
    
    // finally, load any discovered or explicitly provided cell files
    for (const cp of this._cellPaths) {
      try { this.loadCell(cp); } catch (e) { if (this.debug) console.warn("vexon: failed loading cell", cp, e); }
    }
    this.debug = !!options.debug;
