...
class VM {
  constructor(consts = [], code = [], options = {}) {
    ...
    // --- cell loader: require the single kernel cell (vexon_cell1.js) ---
try {
  const coreDir = __dirname;
  const cellPath = require("path").join(coreDir, "vexon_cell1.js");
  // require the kernel cell next to the core if present, and integrate it.
  if (require("fs").existsSync(cellPath)) {
    const mod = require(cellPath);
    if (typeof mod === "function") {
      // typical export: module.exports = function(vm) { ... }
      mod(this);
    } else if (mod && typeof mod.init === "function") {
      mod.init(this);
    } else {
      // support plain object exports with builtins/globals
      if (mod && typeof mod.builtins === "object") {
        for (const k of Object.keys(mod.builtins)) this.builtins[k] = mod.builtins[k];
      }
      if (mod && typeof mod.globals === "object") {
        for (const k of Object.keys(mod.globals)) this.globals.set(k, mod.globals[k]);
      }
    }
    if (this.debug) console.log("vexon: loaded kernel cell vexon_cell1.js");
  } else {
    if (this.debug) console.warn("vexon: vexon_cell1.js not found next to core, skipping kernel cell load");
  }
} catch (e) {
  if (this.debug) console.warn("vexon: failed loading vexon_cell1.js", e);
}
        ...
        switch (inst.op) {
          ...
          case Op.CALL: {
            ...
            if (callee === undefined || callee === null) {
              // Convert JS-level call-of-undefined into a language-level throw so `try/catch` in Vexon catches it.
              const ex = "Call of undefined or null";
              let handled = false;
              while (this.frames.length > 0) {
              ...
              if (!handled) {
                throw new Error("Uncaught exception: " + String(ex));
              }
              break;
            }
...
// --- Vexon cell loader API ---
/**
 * Load a JS "cell" module file and allow it to initialise the VM.
 * A cell may export:
 *   - function(vm) { ... }
 *   - { init(vm) { ... } }
 *   - plain object with properties 'builtins' and/or 'globals'
 */
VM.prototype.loadCell = function(cellPath) {
  try {
    const abs = require("path").resolve(this.baseDir, cellPath);
    if (!require("fs").existsSync(abs)) {
      if (this.debug) console.warn("vexon: cell not found:", abs);
      return;
    }
    ...
    if (typeof mod === "function") {
      ...
      if (mod && typeof mod.builtins === "object") {
        for (const k of Object.keys(mod.builtins)) {
          this.builtins[k] = mod.builtins[k];
        }
      }
      if (mod && typeof mod.globals === "object") {
        for (const k of Object.keys(mod.globals)) {
          this.globals.set(k, mod.globals[k]);
        }
      }
    }
    if (this.debug) console.log("vexon: loaded cell:", abs);
  } catch (e) {
    console.error("vexon: error loading cell", cellPath, e && e.stack ? e.stack : e);
  }
};
